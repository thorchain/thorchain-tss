...
func TestSignatureNotifierHappyPath(t *testing.T) {
	...
	assert.Nil(t, n2.BroadcastSignature(messageID, []*signing.SignatureData{&signature}, []peer.ID{
		p1, p3,
	}))
	assert.Nil(t, n3.BroadcastSignature(messageID, []*signing.SignatureData{&signature}, []peer.ID{
		p1, p2,
	}))
	wg.Wait()
}

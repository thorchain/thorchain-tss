...
func (n *Notifier) verifySignature(data *signing.SignatureData, msg []byte) (bool, error) {
	...
	return ecdsa.Verify(pub.ToECDSA(), msg, new(big.Int).SetBytes(data.GetSignature().R), new(big.Int).SetBytes(data.GetSignature().S)), nil
}
...
func (n *Notifier) ProcessSignature(data []*signing.SignatureData) (bool, error) {
	...
	for i := 0; i < len(data); i++ {
		...
		if sig.GetSignature() != nil {
			sig = data[i]
			verify, err := n.verifySignature(sig, msg)
			if err != nil {
